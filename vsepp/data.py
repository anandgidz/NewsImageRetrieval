...
class GoodNewsHeadlines(torch.utils.data.Dataset):

    def __init__(self, vocab, headlines=True, transform=None):
        self.transform = transform
        f = open("../../project/test_set.json")
        data = json.load(f)
        self.img_headline_pair = [(f'../../project/resized/{k}', data[k]['headline'], data[k]['caption']) for k in data.keys()]
    ...
    def __getitem__(self, index):
        ...
        if self.headlines:
            text = self.img_headline_pair[index][1]
        else:
            text = self.img_headline_pair[index][2]
        ...
        return image, target, index, 0

    # Return the total number of samples.
    def __len__(self):
        return int(len(self.img_headline_pair))
...
def getTestLoader(vocab, headlines, batch_size = 128):
    transform = getTestTransform()

    dataset = GoodNewsHeadlines(vocab, headlines=headlines, transform=transform)
