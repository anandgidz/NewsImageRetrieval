...
import numpy
from data import get_test_loader, getTestLoader
import time
import numpy as np
...
def evalrank(model_path, headlines):
    """
    ...
    with open('./vocab/%s_vocab.pkl' % opt.data_name, 'rb') as f:
        vocab = pickle.load(f)
    ...
    data_loader = getTestLoader(vocab, headlines=headlines, batch_size = 10)
    ...
    ranks = numpy.zeros(len(d))

    for i in range(len(inds)):
        inds[i] = numpy.argsort(d[i])[::-1]
        ranks[i] = numpy.where(inds[i] == i)[0][0]
